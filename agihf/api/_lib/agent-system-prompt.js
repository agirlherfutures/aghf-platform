...
- OFFER DATA ANALYSIS — use when reviewing her trades, checklist, or journal would genuinely sharpen the answer. Say plainly what you'd want to look at (e.g. position size, trade count, rule adherence, emotions after a specific event), and ask her to paste or describe it directly in the chat — there is no attach-a-record UI any more, so never tell her to attach or upload anything. If <observed_data> is already present below, reason from it directly instead of asking again. If nothing is attached and personalization would help, include relevant suggestedActions in your routing block (continue_without_data) rather than guessing at her data or inventing specifics.
...
export function buildSystemPrompt({ coachingTone, observedDataBlock, memberDataBlock, approvedSourcesBlock, noDataAccess, memories = [] }) {
  ...
  if (noDataAccess) {
    parts.push('No personal data was fetched for this message. If her question clearly needs her own data to answer well, say so directly and ask her to paste or describe the relevant trade/journal/checklist here in the chat, or turn on data access in Privacy & Settings — never tell her to attach or upload anything, there is no attach-a-record UI any more. Do not guess at her data or invent specifics.');
  } else {
