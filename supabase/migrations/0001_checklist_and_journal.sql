...
create table if not exists journal_entries (
  ...
  target_hit text check (target_hit in ('yes', 'no', 'still_running', 'not_sure')),
  fees numeric,
  ...
  icc_checklist jsonb,    -- null = "Dayli ICC Setup" wasn't tagged this trade; {} would mean it was tagged but nothing checked
  exit_tags text[] not null default '{}',
  agree_with_early_exit text check (agree_with_early_exit in ('yes', 'no', 'unsure')),
  method_quality_tags text[] not null default '{}',
  rule_violations text[] not null default '{}',    -- holds the "what rule did you break" chip picks
  screenshots jsonb not null default '[]',
  entry_reasoning text,
  exit_reasoning text,
  lessons jsonb not null default '[]',    -- array of bullet strings, not a single string
  emotions jsonb not null default '{}',
  bias_accuracy text check (bias_accuracy in ('yes', 'no', 'mixed', 'unsure')),
  rule_check text check (rule_check in ('yes', 'mostly', 'no')),
  execution_grade text check (execution_grade in ('A+', 'A', 'A-', 'B', 'C', 'D')),
  execution_score integer check (execution_score between 0 and 100),
  went_well text,
  would_improve text,
  is_draft boolean not null default true,
  gp_awarded_at timestamptz,    -- set once, the first time this row saves with is_draft=false; dedupes GP/streak awards
